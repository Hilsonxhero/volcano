...
class MetaHelper
{
    ...
    {
        if ($value === null) {
            return MetaType::META_TYPE_NULL;
        }
        if (is_array($value)) {
            return MetaType::META_TYPE_ARRAY;
        }
        if (
            is_string($value) &&
            ...
            return MetaType::META_TYPE_STRING;
        }
        if (is_string($value) && (is_object(json_decode($value)) || is_array(json_decode($value)))) {
            return MetaType::META_TYPE_JSON;
        }
        if ($value instanceof Collection || $value instanceof \Illuminate\Support\Collection) {
            return MetaType::META_TYPE_COLLECTION;
        }
        if (($value === true || $value === false) && $value !== 1 && $value !== 0) {
            return MetaType::META_TYPE_BOOLEAN;
        }
        if (preg_match('/^[0-9\.]+$/', $value)) {
            return MetaType::META_TYPE_INTEGER;
        }
        return MetaType::META_TYPE_STRING;
    }
    ...
    protected static function convertToCollectionType($value)
    {
        if (is_string($value) && (is_object(json_decode($value)) || is_array(json_decode($value)))) {
            ...
            $value = ($value instanceof Collection || $value instanceof \Illuminate\Support\Collection
            ) ? $value : collect([]);
        }
        return $value;
    }
