...
abstract class SetMetaHelper
{
    ...
    public static function determineMetaTypes($key, $value, $type)
    {
        ...
        if ($type === null && !is_array($key)) {
            ...
            if ($type == MetaType::META_TYPE_COLLECTION || $type == MetaType::META_TYPE_ARRAY || $type == MetaType::META_TYPE_JSON) {
                $type = MetaType::META_TYPE_COLLECTION;
            }
        } elseif (is_array($key)) {
            foreach ($key as $keyItem => $keyItemValue) {
                if ($value === null) {
                    $thisItemType = Meta::guessType($keyItemValue);
                    if ($thisItemType == MetaType::META_TYPE_COLLECTION || $thisItemType == MetaType::META_TYPE_ARRAY || $thisItemType == MetaType::META_TYPE_JSON) {
                        $thisItemType = MetaType::META_TYPE_COLLECTION;
                    }
    ...
    public static function convertArrayAndCollectionsMetaToJson($key, $value, $type, $types)
    {
        if (is_array($key)) {
            foreach ($key as $keyItem => $keyItemValue) {
                if ($types[$keyItem] == MetaType::META_TYPE_COLLECTION || $types[$keyItem] == MetaType::META_TYPE_ARRAY || $types[$keyItem] == MetaType::META_TYPE_JSON) {
                    $key[$keyItem] = Meta::convertMetaValueToType($keyItemValue, MetaType::META_TYPE_JSON);
                } else {
            ...
            if ($type == MetaType::META_TYPE_COLLECTION || $type == MetaType::META_TYPE_ARRAY || $type == MetaType::META_TYPE_JSON) {
                $value = Meta::convertMetaValueToType($value, MetaType::META_TYPE_JSON);
            } else {
    ...
    public static function executeCreateMeta(
        ...
        if (!is_array($key)) {
            ...
            $currentMetaItem = [];
            foreach ($key as $keyItem => $keyItemValue) {
                $currentMetaItemTemporary = new EntitiesMeta();
